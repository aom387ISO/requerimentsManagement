...
router.post('/editarProyecto', async (req, res) => {
    ...
    let { nombreProyecto, esfuerzo, proyectoId} = req.body.almacenVariables;
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        esfuerzo = esfuerzo !== undefined ? esfuerzo : proyectoExistente.esfuerzo;
          ...
          'UPDATE Proyecto SET nombreProyecto = ?, prioridad = ?, esfuerzo = ?, estaEliminado = ? WHERE idProyecto = ?',
          [nombreProyecto, -1, esfuerzo, false, proyectoId]
        );
        res.status(200).json({
          success: true,
